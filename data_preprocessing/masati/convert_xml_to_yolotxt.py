...
def convert_xml_annotation_to_yolo(label_path, output_path, is_masati):
    ...
    for obj in root.iter('object'):
        ...
        boundaries = (float(xmlbox.find('xmin').text), float(xmlbox.find('xmax').text), float(xmlbox.find('ymin').text), float(xmlbox.find('ymax').text))
        bounding_box = yolo_conversion((width,height), boundaries)
        out_file.write(str(class_id) + " " + " ".join([str(a) for a in bounding_box]) + '\n')
...
def main():
    ...
    for label in progress_bar(os.listdir(labels_dir)):
        convert_xml_annotation_to_yolo(os.path.join(labels_dir,label), output_path,  True)
